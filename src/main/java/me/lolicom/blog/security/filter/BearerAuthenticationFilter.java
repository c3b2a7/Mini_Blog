package me.lolicom.blog.security.filter;
...
public class BearerAuthenticationFilter extends OncePerRequestFilter {
    ...
    public BearerAuthenticationFilter(String headerName, String tokenPrefix) {
        ...
        this.authenticationConverter =
                new BearerAuthenticationConverter(tokenPrefix, new WebAuthenticationDetailsSource());
    }

    protected String getJwtToken(HttpServletRequest request) {
        String authInfo = request.getHeader(headerName);
        return StringUtils.trimLeadingWhitespace(authInfo.substring(tokenPrefix.length()));
    }
        ...
        try {
            ...
            if (requiredAuthentication(request)) {
                if (this.logger.isDebugEnabled()) {
                    logger.debug(
                            "Bearer Authentication Authorization header found for token[" + authRequest.getName() + "]");
                }
